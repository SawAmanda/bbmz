<template>
    <section>
        <div class="searchBox">
            ...
            <el-button type="danger" class="el-button--small" @click="sendTime">搜索</el-button>
        </div>
        <table>
            <tr class="desc">
                <td>序号</td>
...
export default {
    ...
    mounted() {
        this.$http({
            method: "get",
            url: "/member/store_record/getMyCenterRecord",
            params: {
        ...
        sendTime() {
            if (this.value6) {
                ...
                this.$http({
                    ...
                    params: {
                        token: window.localStorage.getItem("token"),
                        start_date: this.value6[0],
                        end_date: this.value6[1]
                    }
                }).then(res=>{
                    for (var i=0;i<res.data.data.length;i++) {
                        var pointdata = res.data.data[i]
