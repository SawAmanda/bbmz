<template>
    <section>
        <p class="pageTips">您当前的动态积分：{{dyn}}， 中心积分：{{cen}}， 当前转换比例：{{bili}}</p>
        <table class="registerNumber">
            <tr class="desc">
                <td colspan="2">请填写您要转换的动态积分</td>
            </tr>
            <tr>
                <td>转换数量：</td>
                <td>
...
export default {
    ...
    mounted() {
        ...
        let ustdata = JSON.parse(window.localStorage.getItem("ust"))
        this.dyn = ustdata.dyn
        this.cen = ustdata.cen
    },
    methods: {
        open4() {
            if (this.input == '' || this.input == this.dyn || this.input == 0) {
                this.$message({
                    showClose: true,
                    message: '请输入正确的转换数量',
                    type: 'error'
                });
            }else if(this.input > this.dyn) {
                this.$message({
                    showClose: true,
                ...
                this.$http({
                    method: "get",
                    url: "/member/member_store/dynamicToCenter",
